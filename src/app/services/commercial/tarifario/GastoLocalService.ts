...
export class GastoLocalService implements ICrud {

    private url = `${environment.api.url}local-expenses`;
    ...
    paginate(page: number, search_text: string = '') {
        return this.http.get(`${this.url}?page=${page}&search_text=${search_text}`);
    }
}